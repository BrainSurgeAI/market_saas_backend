...
CREATE TABLE `order_details` (
  ...
  `accepted_quantity` decimal(8,2) DEFAULT '0.00' COMMENT '实际数量，供应商称重后更新',
  `original_price` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '原始单价',
  `discount_rate` decimal(4,2) NOT NULL DEFAULT '1.00' COMMENT '折扣率',
  `actual_price` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '实际单价（折扣后）',
  `total_amount` decimal(12,2) NOT NULL DEFAULT '0.00' COMMENT '总金额（数量*实际单价）',
  `actual_amount` decimal(12,2) DEFAULT '0.00' COMMENT '实际总金额（实际数量*实际单价）',
  ...
  `receipt_quantity` decimal(8,2) DEFAULT NULL COMMENT '实际签收数量',
  `receipt_date` timestamp NULL DEFAULT NULL COMMENT '签收时间',
  `receipt_notes` varchar(255) DEFAULT NULL COMMENT '签收备注',
  `receipt_evidence` varchar(255) DEFAULT NULL COMMENT '签收凭证(图片URL)',
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `order_inspections` (
  ...
  `inspected_by_type` enum('MARKET','CUSTOMER') COLLATE utf8mb4_unicode_ci NOT NULL,
  `inspected_by_id` int NOT NULL,
  `inspection_result` enum('PENDING','PASS','PARTIAL','REJECTED') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'PENDING',
...
CREATE TABLE `orders` (
  ...
  `actual_amount` decimal(12,2) NOT NULL DEFAULT '0.00' COMMENT '实际支付金额',
...
CREATE TABLE `provider_deliveries` (
  ...
  `delivery_status` enum('PREPARING','DELIVERED','CANCELLED') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'PREPARING' COMMENT '发货状态',
...
CREATE TABLE `provider_delivery_items` (
  ...
  `weight_unit` varchar(8) COLLATE utf8mb4_unicode_ci DEFAULT 'kg' COMMENT '计量单位',
  `remark` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '备注',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `delivery_round` int unsigned DEFAULT NULL,
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='供应商发货明细表（商品维度）';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `reconciliation_statement_details` (
  ...
  `product_code` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '产品编号',
...
CREATE TABLE `reconciliation_statement_orders` (
  ...
  `statement_id` int NOT NULL COMMENT '对账单ID',
  `order_id` int NOT NULL COMMENT '订单ID',
  `order_code` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '订单编号',
  `order_date` date NOT NULL COMMENT '订单日期',
...
CREATE TABLE `reconciliation_statements` (
  `id` int NOT NULL AUTO_INCREMENT,
  `statement_code` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '对账单编号',
  `customer_id` int NOT NULL COMMENT '客户ID',
  `market_id` int NOT NULL COMMENT '市场ID',
  `provider_id` int NOT NULL COMMENT '供应商ID',
  `supplier_name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '供应商名称',
  ...
  `confirmed_at` timestamp NULL DEFAULT NULL COMMENT '确认时间',
  `completed_by` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '完成处理人',
  `completed_at` timestamp NULL DEFAULT NULL COMMENT '完成时间',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
) ENGINE=InnoDB AUTO_INCREMENT=26351 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
