...
impl MySqlRepository {
    ...
    async fn verify_order_amount(
        ...
        debug!(
            "开始验证订单金额: 客户ID={}, 订单={:?}",
            customer_id, order_payload
        );
        // 解析配送日期，用于查询折扣的有效期
        let delivery_date =
            NaiveDate::parse_from_str(&order_payload.delivery_info.delivery_date, "%Y-%m-%d")
                .map_err(|e| {
                    error!("配送日期格式错误: {:#?}", e);
        ...
        // 查询所有相关的折扣信息
        let level1_category_ids: Vec<i32> = category_to_level1.values().copied().collect();
        ...
        for discount in discounts {
            ...
            discount_map
                .entry(discount.category_id)
                .or_insert(discount.discount_rate);
        }
        ...
        for item in &order_payload.items {
            // 获取一级分类ID
            let level1_category_id =
                category_to_level1.get(&item.category_id).ok_or_else(|| {
                    AppError::bad_request(format!("无法找到商品 {} 的一级分类", item.product_code))
                })?;
...
impl CustomerOrderRepository for MySqlRepository {
    async fn create_order(
        ...
        // 验证订单金额
        self.verify_order_amount(record.customer_id, order_payload)
            .await?;

        // Pre-calculate order amounts for efficiency
        let total_amount: Decimal = order_payload
            .items
            .iter()
