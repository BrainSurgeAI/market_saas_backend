    ...
    T: SystemLogRepository + Send + Sync,
{
    ...
    pub async fn log_info(&self, log: SystemLogDTO) -> Result<(), AppError> {
        self.repo.log_info(log).await
    }

    pub async fn log_error(&self, log: SystemLogDTO) -> Result<(), AppError> {
    ...
    // 计时器上下文，用于记录执行时间
    pub fn timer(&self) -> OperationTimer {
        OperationTimer {
            start: Instant::now(),
        }
    }
